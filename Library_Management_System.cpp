...
class student
{
    ...
    char name[20];
    char stbooknumber[6]; // student book no
...
public:
    ...
    void getstbooknumber(char t[])
    {
        strcpy(stbooknumber, t);
    }
    void report()
    {
...
void displayspb(char n[])
{
    ...
    {
        if (strcmpi(bk.retbooknumber(), n) == 0) // not case sensitive
        {
            bk.showbook();
            flag = 1;
        }
...
void modifybook()
{
    ...
    {
        if (strcmpi(bk.retbooknumber(), n) == 0)
        {
            bk.showbook();
    ...
    while (fp.read((char *)&st, sizeof(student)) && found == 0)
    {
        if (strcmpi(st.retadmno(), sn) == 0) // compare admsn no.
        {
            ...
            if (st.rettoken() == 0) // if book not issued
            {
                ...
                {
                    if (strcmpi(bk.retbooknumber(), bn) == 0) // compare book no.
                    {
                        flag = 1;
                        st.addtoken();
                        st.getstbooknumber(bk.retbooknumber()); // pass book no.
                        int pos = -1 * sizeof(st);
    ...
    while (fp.read((char *)&st, sizeof(student)) && found == 0)
    {
        if (strcmpi(st.retadmno(), sn) == 0) // compare admsn no.
        {
            ...
            if (st.rettoken() == 1) // if book issued
            {
                while (fp1.read((char *)&bk, sizeof(book)) && flag == 0)
                {
                    if (strcmpi(bk.retbooknumber(), st.retstbooknumber()) == 0)
                    {
                        flag = 1;
                        bk.showbook();
